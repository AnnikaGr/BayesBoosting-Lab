...
def plotGaussian(X,y,mu,sigma):
    ...
    for label in labels:
        ...
        plt.scatter(Xclass[:,0],Xclass[:,1],linewidths=1,s=25,color=colors[label],marker='o',alpha=0.75)
        plt.plot(mu[label][0], mu[label][1], marker='x', markersize=3, color="black")
        c += 1.
...
def testClassifier(classifier, dataset='iris', dim=0, split=0.7, ntrials=100, plot = False, correlation = True):
    ...
    if(plot==True):
        plotAccuracy(means)
    if(correlation==True):
        calcFeatureCorrelation(X,y)
        #print("Correlation among features: " + calcFeatureCorrelation(X,y))
...
def calcFeatureCorrelation(X,y):
    classes= np.unique(y)
    for jdx, c in enumerate(classes):
        idx= np.where(y==c)[0]
        values = np.array(X)[idx]
        df = pd.DataFrame(values)
        print("Correlation Matrix - Class: "+str(c)+tabulate(df.corr(), headers='keys', tablefmt='psql'))
    return
